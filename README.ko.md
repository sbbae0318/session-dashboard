...
| OpenCode CLI | oc-serve(헤드리스 서버)에 필요 |
| Bun | TUI 전용 (선택) |
...
```bash
./install/install.sh --agent-only      # Agent + oc-serve만 설치 (Docker 서버 제외)
./install/install.sh --server-only     # Server만 설치 (oc-serve + Agent 제외)
./install/install.sh --oc-serve-only   # oc-serve만 설치 (Agent + Server 제외)
...
│   └── oc-serve.sh  # oc-serve (OpenCode 헤드리스) 설치/관리
├── docs/            # 아키텍처 및 운영 문서
└── README.md
```
...
### oc-serve (OpenCode 헤드리스)

```bash
./install/oc-serve.sh               # 설치 (opencode 확인 + 시작)
./install/oc-serve.sh --status      # 상태 확인 (PID + 헬스 체크)
./install/oc-serve.sh --start       # oc-serve 시작
./install/oc-serve.sh --logs        # 최근 로그 보기 (tail -50)
./install/oc-serve.sh --restart     # 재시작
./install/oc-serve.sh --stop        # 중지
...
> oc-serve가 아직 실행 중이 아니면 먼저 시작하세요: `./install/oc-serve.sh`
