...
function buildInitialPrompt(
  ...
  return `다음은 코딩 세션의 사용자 프롬프트 목록입니다.
이 세션에서 무슨 작업을 했는지 간결하게 요약하세요.

규칙:
- 한국어로 작성
- 2~4문장: 무엇을 했고, 현재 상태는 무엇인지
- 작업 흐름을 시간 순서로 서술
- 핵심 작업만 언급 (사소한 것 제외)
...
function buildAdditivePrompt(
  ...
  return `기존 요약에 새 활동을 통합하여 누적 요약을 작성하세요.

규칙:
- 한국어로 작성
- 3~5문장. 작업 흐름을 시간 순서대로 유지
- "A 작업 후 B를 진행" 형태의 additive 서술
- 이전 요약의 핵심은 유지하되 새 활동으로 업데이트
- 현재 상태 (완료/진행중/블로킹) 명시
