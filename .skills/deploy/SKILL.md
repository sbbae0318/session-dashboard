...
version: 1.0.0
description: |
  session-dashboard 프로젝트 배포 워크플로우 skill.
  테스트 장비(192.168.0.63) 검증 후 운영 장비(192.168.0.2)에 반영하는 전체 파이프라인.
  Triggers: commit, deploy, 배포, 반영, 운영, release, ship, merge to main,
  "테스트 서버", "운영 서버", PR 생성, 기능 완료 시 자동 활성화.
---
...
| 환경 | 호스트 | 대시보드 URL | Agent 포트 | 용도 |
|------|--------|-------------|-----------|------|
| **테스트** | 192.168.0.63 | http://192.168.0.63:3097 | 3101 | 개발/검증 (MacBook) |
| **운영** | 192.168.0.2 | http://192.168.0.2:3097 | 3098 | 프로덕션 (Workstation) |
...
**GATE**: 유닛 테스트 전체 통과 필수. 실패 시 다음 단계 진행 금지.
...
> Dockerfile Stage 2에서 `npm test`가 빌드 게이트로 실행됨. 테스트 실패 시 빌드 실패.
...
**GATE**: E2E 통과 + 수동 검증 완료 필수. 실패 시 운영 배포 금지.
...
> main 직접 push 대신 feature branch + PR 권장.
...
```bash
# --- 테스트 장비 (192.168.0.63, 로컬) ---
cd server && docker compose up -d --build          # server 재배포
./install/agent.sh --restart                       # agent 재시작
curl -sf http://192.168.0.63:3097/health           # server 헬스
curl -sf http://192.168.0.63:3101/health           # agent 헬스
...
./install/agent.sh --restart
curl -sf http://192.168.0.2:3097/health
curl -sf http://localhost:3098/health
```
