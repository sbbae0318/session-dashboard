...
2. **Pre-existing 19개 테스트 실패** 조사 — `claude-heartbeat.test.ts`의 eviction/PID liveness/parseConversationFile 계열.
3. **정렬 변경 라이브 검증** — WAITING/WORKING 세션이 IDLE 위에 항상 표시되는지 확인.
