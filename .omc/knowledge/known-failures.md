...
| F-007 | 스피너/flash 애니메이션 실제 브라우저 미동작 (headless 정상) | 2중 원인: (1) Svelte 5 scoped `@keyframes` 해시 접두사가 실제 브라우저에서 미적용 (2) `@media(prefers-reduced-motion:reduce)` 규칙이 실제 브라우저에서 매칭되어 `animation:none` 덮어씀. Playwright headless는 기본 `no-preference`로 진단 불일치 유발. DevTools console `getComputedStyle(span).animationName==="none"`으로 확인 | (1) `@keyframes`+`animation` CSS를 `app.css` 글로벌로 이동 (2) `prefers-reduced-motion` 규칙 제거. **규칙: 컴포넌트 `<style>` 내 `@keyframes` 금지. Docker 배포 시 `down + up --build --force-recreate` 필수 (`--no-cache`만 불충분)** | High | `server/frontend/src/app.css` (480bf80) |
