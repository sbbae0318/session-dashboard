import type { Memo, MemoWithContent } from '../../types';
...
export async function fetchMemos(projectId?: string, date?: string): Promise<void> {
  loading = true;
  try {
...
export async function createMemo(projectId: string, content: string, title?: string, date?: string): Promise<Memo | null> {
  saving = true;
  try {
    const body: Record<string, string> = { projectId, content };
