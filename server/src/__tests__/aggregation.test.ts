...
describe('Multi-machine data aggregation', () => {
  ...
  describe('Cards aggregation (pollAllCards)', () => {
    it('should merge cards from 2 machines and sort by startTime desc', async () => {
      setupUrlRouter({
        'http://10.0.0.1:3100/api/cards': JSON.stringify({
          cards: [
            { sessionId: 'a-1', startTime: 1000, summary: 'First from A' },
            { sessionId: 'a-2', startTime: 5000, summary: 'Latest from A' },
          ],
        }),
        'http://10.0.0.2:3100/api/cards': JSON.stringify({
          cards: [
            { sessionId: 'b-1', startTime: 3000, summary: 'From B' },
          ],
        }),
      });
      ...
      // Sorted desc: 5000, 3000, 1000
      expect(cards).toHaveLength(3);
      expect(cards[0].sessionId).toBe('a-2');
      expect(cards[0].startTime).toBe(5000);
      expect(cards[1].sessionId).toBe('b-1');
      expect(cards[1].startTime).toBe(3000);
      expect(cards[2].sessionId).toBe('a-1');
      expect(cards[2].startTime).toBe(1000);
    });

    it('should tag each card with machineId, machineAlias, machineHost', async () => {
      setupUrlRouter({
        'http://10.0.0.1:3100/api/cards': JSON.stringify({
          cards: [{ sessionId: 'a-1', startTime: 1000 }],
        }),
        'http://10.0.0.2:3100/api/cards': JSON.stringify({
          cards: [{ sessionId: 'b-1', startTime: 2000 }],
        }),
      });

      const cards = await manager.pollAllCards();

      const cardA = cards.find(c => c.sessionId === 'a-1');
      ...
      const cardB = cards.find(c => c.sessionId === 'b-1');
    ...
    it('should not break aggregation when one machine returns empty', async () => {
      setupUrlRouter({
        'http://10.0.0.1:3100/api/cards': JSON.stringify({ cards: [] }),
        'http://10.0.0.2:3100/api/cards': JSON.stringify({
          cards: [{ sessionId: 'b-1', startTime: 1000 }],
        }),
      });
      ...
      expect(cards[0].machineId).toBe('machine-b');
    });

    it('should silently skip failed machine and return other cards', async () => {
      setupUrlRouter(
        {
          'http://10.0.0.2:3100/api/cards': JSON.stringify({
            cards: [{ sessionId: 'b-1', startTime: 2000 }],
