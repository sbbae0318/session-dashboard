...
function killProcess(pid: number, label: string): void {
  try {
    ...
    console.log(`[E2E OC Teardown] Sent SIGTERM to ${label} (PID: ${pid})`);
    ...
    setTimeout(() => {
      try {
        ...
        console.log(`[E2E OC Teardown] Sent SIGKILL to ${label} (PID: ${pid})`);
      } catch {
        // Already dead — good
      }
    }, 3000);
  } catch {
    console.log(`[E2E OC Teardown] ${label} (PID: ${pid}) already stopped`);
  }
}

export default async function globalTeardown(): Promise<void> {
  console.log('[E2E OC Teardown] Cleaning up...');
  ...
  if (existsSync(PID_FILE)) {
    try {
      ...
      console.error('[E2E OC Teardown] Failed to read PID file:', err);
    }
  }
  ...
  for (const dir of [TEST_AGENT_HOME, TEST_SERVER_HOME]) {
    if (existsSync(dir)) {
      ...
      console.log(`[E2E OC Teardown] Removed ${dir}`);
    }
  }

  console.log('[E2E OC Teardown] Done.');
}
