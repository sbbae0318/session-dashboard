...
export default async function globalSetup(): Promise<void> {
  console.log('[E2E OC Setup] Starting OpenCode regression test environment...');
  ...
  if (existsSync(lockFile)) {
    ...
    console.log('[E2E OC Setup] Removed stale lock file');
  }
  ...
  agentProc.stdout?.on('data', (d: Buffer) => process.stdout.write(`[oc-agent] ${d}`));
  agentProc.stderr?.on('data', (d: Buffer) => process.stderr.write(`[oc-agent:err] ${d}`));
  ...
  serverProc.stdout?.on('data', (d: Buffer) => process.stdout.write(`[oc-server] ${d}`));
  serverProc.stderr?.on('data', (d: Buffer) => process.stderr.write(`[oc-server:err] ${d}`));
  ...
  try {
    ...
    console.log(`[E2E OC Setup] Test agent ready on port ${AGENT_PORT} (PID: ${agentProc.pid})`);
  } catch (err) {
    console.error('[E2E OC Setup] Agent failed to start');
    throw err;
  }

  try {
    await waitForHealth(`http://127.0.0.1:${SERVER_PORT}/health`);
    console.log(`[E2E OC Setup] Test server ready on port ${SERVER_PORT} (PID: ${serverProc.pid})`);
  } catch (err) {
    console.error('[E2E OC Setup] Server failed to start');
    throw err;
  }

  console.log('[E2E OC Setup] Environment ready.');
}
